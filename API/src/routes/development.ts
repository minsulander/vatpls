...
let predefinedControllers = [
    ...
    { name: "Max Kuhla", sign: "MK", cid: "1157126", rating: "S1", callsign: "", frequency: "", position: "", timestamp: dayjs.utc().format() },
    {
        name: "Test Testsson",
        sign: "T2",
        cid: "931204",
...
const isValidMoveController = (controllerObj: any): { cid: string; position: string; callsign: string } | undefined => {
    if (typeof controllerObj.cid === "string" && typeof controllerObj.position === "string" && typeof controllerObj.callsign === "string") {
        return {
...
const addControllerToList = (controllerToAdd: Controller) => {
    if (controllerToAdd.position == "paus") {
        availableControllers.push({ ...controllerToAdd, position: "paus", callsign: "paus", timestamp: dayjs.utc().format() })
    } else if (controllerToAdd.position == "other") {
        awayControllers.push({
            ...controllerToAdd,
            position: controllerToAdd.callsign,
            callsign: controllerToAdd.callsign,
            timestamp: dayjs.utc().format(),
        })
    } else {
        ...
        activeControllers.push({
            ...controllerToAdd,
            position: controllerToAdd.position,
            callsign: controllerToAdd.callsign,
            timestamp: dayjs.utc().format(),
        })
    }
    ...
    if (Array.isArray(newActiveControllers) && Array.isArray(newAvailableControllers) && Array.isArray(newAwayControllers)) {
        ...
        activeControllers = newActiveControllers.map((controller) => {
            ...
            return controller.cid === movedController.cid
                ? { ...controller, timestamp: dayjs.utc().format() } // Reset timestamp when moving to active
                : controller
        })
        availableControllers = newAvailableControllers.map((controller) => {
            ...
            return controller.cid === movedController.cid
                ? { ...controller, timestamp: dayjs.utc().format() } // Reset timestamp when moving to paus
                : controller
        })
        awayControllers = newAwayControllers.map((controller) => {
            ...
            return controller.cid === movedController.cid
                ? { ...controller, timestamp: dayjs.utc().format() } // Reset timestamp when moving to other
                : controller
        })
        ...
        let { position: newposition, callsign: newcallsign, name, sign, rating, frequency, cid } = movedController
        ...
        if (newposition == "paus") {
            availableControllers.push({ ...movedController, position: "paus", callsign: "paus", timestamp: dayjs.utc().format() })
        } else if (newposition == "other") {
            awayControllers.push({ ...movedController, position: "other", callsign: "other", timestamp: dayjs.utc().format() })
        } else {
            // position is an active position
            activeControllers.push({ ...movedController, position: newposition, callsign: newcallsign, timestamp: dayjs.utc().format() })
        }
        res.status(200).send("Controllers updated successfully")
    } else {
