...
export async function getOneController(req: Request, res: Response) {
    try {
        const result = await query_database("SELECT * FROM Controller WHERE cid = $1", [req.params.id.toString()])
...
const checkControllerExists = async (cid: string) => {
    const controller = await query_database("SELECT * FROM Controller WHERE cid = $1", [cid])
